# Changelog

## Unreleased

- 修正 momo 主商品價格解析順序，優先使用 `meta name="product:price:amount"`，避免抓到頁面內加購/組合商品的 `goodsPrice`。
- 新增 momo 價格回歸測試，覆蓋主商品 meta 價格優先於內嵌 `goodsPrice` 的情境。

## 2026-04-25

- 建立 PChome / momo 每日價格爬蟲。
- 加入 GitHub Actions 雲端排程與 Telegram 通知。
- 設定 GitHub repo secrets：`TELEGRAM_BOT_TOKEN`、`TELEGRAM_CHAT_ID`。
- 修正排程報告行為：即使部分商品抓取失敗，仍會送出 Telegram 報告。
- 加入 momo 抓取失敗診斷，若遇到逾時、HTTP 403/429/503、驗證頁或阻擋頁，會在報告標示疑似反爬蟲/限流。
- 將每日自動抓取時間由台北時間 12:45 調整為 12:00。
- 因 GitHub Actions 整點排程可能延遲或被丟棄，將每日自動抓取時間由台北時間 12:00 調整為 12:07。
- 因 2026-04-26 的 GitHub scheduled run 未觸發，新增 12:17、12:27 備援排程與同日去重保護，避免漏發或重複發 Telegram。
- 因 2026-04-27 的 12:07、12:17 仍未準時觸發，改成台北時間 12:00-13:55 每 5 分鐘嘗試一次，並維持同日去重保護。
- 依使用者要求，撤回每 5 分鐘補跑機制，恢復每日台北時間 12:07 單一排程。
- 移除搭配每 5 分鐘補跑使用的同日去重腳本，讓 workflow 回到單一 12:07 直接執行。
- 修正 PChome 價格欄位選擇順序，改為優先抓取折扣價 `Low`，避免 SK-II 青春露抓到原價而漏掉特價。
- 新增 PChome 選價回歸測試，確認 `Low -> P -> M` 的 fallback 順序。
- 將大研生醫 B 群比較來源改為新的 PChome / momo 商品頁，並維持抓最低價。
